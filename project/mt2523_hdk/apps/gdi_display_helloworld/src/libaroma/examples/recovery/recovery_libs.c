...
LIBAROMA_STREAMP recovery_stream_uri_cb(char * uri) {
	...
	if (strcmp(kwd, "res://") == 0) {
		...
		if (ret) {
			/* change uri */
			snprintf(ret->uri, LIBAROMA_STREAM_URI_LENGTH, "%s", uri);
			return ret;
		}
	}
	return NULL;
...
void recovery_statusbar_sidebar_list_cb(LIBAROMA_CONTROLP ctl,
		LIBAROMA_CANVASP cv, int sy) {
...
void recovery_statusbar_update() {
	word bgcolor = libaroma_alpha(_recovery.status_bgcolor, 0,
			51 + recovery_statusbar_side_opa);
	byte isdark = libaroma_color_isdark(bgcolor);
	word text_color = isdark ? 0xffff : 0;

	libaroma_canvas_fillcolor(_recovery.status_canvas, bgcolor);

	if (recovery_statusbar_side_w) {
		if (recovery_statusbar_overlay_canvas) {
			libaroma_draw_rect(_recovery.status_canvas, 0, 0,
					recovery_statusbar_side_w, _recovery.status_canvas->h, 0,
					0xff);
			libaroma_draw_ex(
			...
			libaroma_draw_rect(_recovery.status_canvas, 0, 0,
					recovery_statusbar_side_w, _recovery.status_canvas->h,
					RGB(cccccc), 0xff);
		}
	}
...
/* statusbar thread */
void recovery_statusbar_ui_thread() {
	if (_recovery.status_updatestart != 0) {
		...
		if (cstate < 1) {
			_recovery.status_bgcolor = libaroma_alpha(
					_recovery.status_prvcolor, _recovery.status_reqcolor,
					255 * cstate);
		} else {
...
byte recovery_init_ui() {
	/* create window */
	RLOG("recovery_init_ui start ..... ");

	_recovery.win = libaroma_window(NULL, 0, 0, LIBAROMA_SIZE_FULL,
			LIBAROMA_SIZE_FULL);
	...
	/* appbar */
	_recovery.appbar = libaroma_ctl_bar(_recovery.win, ID_APPBAR, 0, 0,
			LIBAROMA_SIZE_FULL, 56, "AROMA Recovery",
	...
	/* fragment */
	_recovery.fragment = libaroma_ctl_fragment(_recovery.win, ID_FRAGMENT, 0,
			56, LIBAROMA_SIZE_FULL, LIBAROMA_SIZE_FULL);
	...
	if (_recovery.sidebar) {
		...
		libaroma_window_sidebar_onslide(_recovery.sidebar,
...
#define _DIV(id) \
      libaroma_listitem_divider(_recovery.sidemenu, id, \
		...
		/* create menu list */
		_recovery.sidemenu = libaroma_ctl_list(_recovery.sidebar, ID_SIDEMENU,
				0, 0, LIBAROMA_SIZE_FULL, LIBAROMA_SIZE_FULL, 0, 0,
				libaroma_colorget(NULL, _recovery.win)->control_bg, 0 /*LIBAROMA_CTL_SCROLL_WITH_SHADOW*/
		);

		libaroma_ctl_scroll_set_min_scroll(_recovery.sidemenu,
				recovery_statusbar_sidebar_list_cb, libaroma_dp(24));

		/* sidebar image */
		libaroma_listitem_image(
				_recovery.sidemenu,
				ID_SIDEMENU_IMAGE,
				libaroma_image_uri("res:///bg/sidebar.jpg"),
				150,
...
byte recovery_init() {
	...
	/* set workspace size */
	libaroma_wm_set_workspace(0, libaroma_dp(24), libaroma_fb()->w,
	...
	_recovery.status_canvas = libaroma_canvas_area(libaroma_fb()->canvas, 0, 0,
			libaroma_fb()->w, libaroma_dp(24));
	...
	/* load zip resource */
	RLOG("recovery_init: libaroma_zip start ...... ");
	_recovery.zip = libaroma_zip("/sdcard/recovery.zip");
	RLOG("recovery_init: libaroma_zip end ...... ");
	if (!_recovery.zip) {
...
LIBAROMA_CANVASP recovery_load_icon_ex(const char * icon_name, word color) {
	...
	if (libaroma_stristr(icon_name, ".svg", strlen(icon_name)) != NULL) {
		...
		snprintf(uri, 256, "res:///res/images/%s.png", icon_name);
	}
...
LIBAROMA_CANVASP recovery_load_icon(const char * icon_name,
	...
	return recovery_load_icon_ex(icon_name,
