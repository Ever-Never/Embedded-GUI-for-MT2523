...
void recovery_mainmenu_reset_appbar(LIBAROMA_WINDOWP win, MAINMENU * var) {
	/* set appbar icon */
	libaroma_ctl_bar_set_icon(recovery()->appbar, NULL, 0,
			LIBAROMA_CTL_BAR_ICON_DRAWER, 0);
	libaroma_ctl_bar_set_title(recovery()->appbar, "Main Menu", 0);
	libaroma_ctl_bar_set_subtitle(recovery()->appbar, NULL, 0);
	libaroma_ctl_bar_set_tools(recovery()->appbar, var->tools, 0);
	libaroma_ctl_bar_update(recovery()->appbar);
}

int libaroma_dialog_list(const char * title, const char * button1,
		const char * button2, LIBAROMA_COLORSETP colorset, byte flags);

/* test open dialog */
void recovery_open_dialog(LIBAROMA_WINDOWP parent) {
	// libaroma_sleep(700);
	recovery_statusbar_setcolor(
			libaroma_alpha(libaroma_colorget(NULL, parent)->primary, 0, 0x70));
	/*
...
/* event loop */
void recovery_mainmenu_pool(LIBAROMA_WINDOWP win, MAINMENU * var) {
	byte onpool = 1;
	LIBAROMA_MSG msg;
	dword command;
	byte cmd;
	word id;
	byte param;
	RLOG("recovery_mainmenu_pool: start ......");
		...
		if (msg.msg == LIBAROMA_MSG_KEY_SELECT) {
			...
			if (id == ID_MENULIST) {
				if (msg.key == ID_MENU_USB) {
					...
					libaroma_ctl_list_item_position(var->menu,
							(LIBAROMA_CTL_LIST_ITEMP) msg.d, &rect, 0);
...
byte recovery_mainmenu_init(LIBAROMA_WINDOWP win, MAINMENU * var) {
	/* create menu list */RLOG("recovery_mainmenu_init: start ......");
	var->menu = libaroma_ctl_list(win, ID_MENULIST, 0, 0, LIBAROMA_SIZE_FULL,
			LIBAROMA_SIZE_FULL, 0, 0, libaroma_colorget(NULL, win)->control_bg,
			LIBAROMA_CTL_SCROLL_WITH_SHADOW);

	/* menu flags */
	word menuflags = LIBAROMA_LISTITEM_MENU_INDENT_NOICON
	...
	word menuflagsb = LIBAROMA_LISTITEM_MENU_INDENT_NOICON
			| LIBAROMA_LISTITEM_MENU_FREE_ICON
			| LIBAROMA_LISTITEM_WITH_SEPARATOR
			| LIBAROMA_LISTITEM_SEPARATOR_TEXTALIGN;
...
#define _DIV(id) \
	...
	LIBAROMA_CANVASP
			cphoto = libaroma_image_uri("file:///data/system/users/0/photo.png");
	if (cphoto) {
		...
		LIBAROMA_CANVASP
				bgside = libaroma_image_uri("file:///sdcard/wall1.jpg");
		if (bgside) {
			libaroma_draw_scale_smooth(dcphoto, bgside, 0, 0, dcphoto->w,
					dcphoto->h, 0, 0, bgside->w, bgside->h);
			libaroma_canvas_free(bgside);
		}

		libaroma_draw_scale_smooth(dcphoto, cphoto, libaroma_dp(16),
				libaroma_dp(56), libaroma_dp(72), libaroma_dp(72), 0, 0,
				cphoto->w, cphoto->h);

		char * namexml = libaroma_stream_to_string(
				libaroma_stream("file:///data/system/users/0.xml"), 1);
		if (namexml) {
			char * nametag = libaroma_stristr(namexml, "<name>",
					strlen(namexml)) + 6;
			if (nametag) {
				char * nexttag = libaroma_stristr(nametag, "</name>",
						strlen(nametag));
				if (nexttag) {
					nexttag[0] = 0;
					char myname[256];
					snprintf(myname, 256, "Hi %s\n<size=4>Welcome to Recovery",
							nametag);
					libaroma_draw_text_ex(
							...
							LIBAROMA_FONT(0,5) | LIBAROMA_TEXT_LEFT
									| LIBAROMA_TEXT_FIXED_INDENT
									| LIBAROMA_TEXT_FIXED_COLOR
									| LIBAROMA_TEXT_NOHR, 100, 1, 2, 0, 0x80,
							libaroma_dp(1), libaroma_dp(1));
				}
			}
			free(namexml);
		}
...
#undef _ITEMB
	...
	RLOG("recovery_mainmenu_init: end ......");
...
void recovery_mainmenu() {
	MAINMENU mainmenu = { 0 };
	RLOG("recovery_mainmenu: init start ......");
	/* init window */
	...
	if (win) {
		...
		libaroma_ctl_bar_tools_set(mainmenu.tools, 0, ID_APPBAR_POWER, "Power",
		...
		if (recovery_mainmenu_init(win, &mainmenu)) {
			...
			/* show animated */
			libaroma_window_anishow(recovery()->win,
					LIBAROMA_WINDOW_SHOW_ANIMATION_FADE, 200);
			recovery_mainmenu_reset_appbar(win, &mainmenu);
			libaroma_ctl_fragment_set_active_window(recovery()->fragment,
				ID_MAINMENU_FRAGMENT, 1, 500, 0, NULL, NULL, NULL);
