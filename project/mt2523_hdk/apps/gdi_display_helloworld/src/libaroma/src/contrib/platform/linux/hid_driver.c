...
byte LINUXHIDRV_init(LIBAROMA_HIDP me) {
	...
	LINUXHIDRV_INTERNALP mi = (LINUXHIDRV_INTERNALP) calloc(
			sizeof(LINUXHIDRV_INTERNAL), 1);
...
byte LINUXHIDRV_getdevclass(int fd) {
	...
	/* check keyboard */
	byte haveKeyboardKeys = LINUXHIDRV_nonzero(keyBitmask, 0,
			LINUXHIDRV_SIZEOF_BIT_ARRAY(BTN_MISC)) || LINUXHIDRV_nonzero(
			keyBitmask, LINUXHIDRV_SIZEOF_BIT_ARRAY(KEY_OK),
			LINUXHIDRV_SIZEOF_BIT_ARRAY(KEY_MAX + 1));

	/* check gamepad */
	byte haveGamepadButtons = LINUXHIDRV_nonzero(keyBitmask,
			LINUXHIDRV_SIZEOF_BIT_ARRAY(BTN_MISC),
			LINUXHIDRV_SIZEOF_BIT_ARRAY(BTN_MOUSE)) || LINUXHIDRV_nonzero(
			keyBitmask, LINUXHIDRV_SIZEOF_BIT_ARRAY(BTN_JOYSTICK),
	...
	if ((dev->devclass & LINUXHIDRV_DEVCLASS_TOUCH)) {
		...
		if (dev->vkn > 0) {
			...
			for (i = 0; i < dev->vkn; i++) {
				...
				int j;
				for (j = 0; j < 6; j++) {
					token[j] = LINUXHIDRV_strtok_r((i || j) ? NULL : vks, ":",
							&ts);
				}
		...
		if (me->internal == NULL) {
			...
			for (n = 0; n < mi->n; n++) {
				if (mi->fds[n].revents & POLLIN) {
					...
					if (r == sizeof(ev)) {
						/* translate it */
						byte translate_ret = LINUXHIDRV_translate(me,
								&mi->dev[n], dest_ev, &ev);
				//		printf("MY TOUCH: T=%i, C=%i, V=%i \n \n", (&ev)->type,
				//				(&ev)->code, (&ev)->value);
