...
void LINUXFBDR_swap_buffer(LINUXFBDR_INTERNALP mi){
  ...
  if ((mi->double_buffering)&&(mi->var.yoffset==0)){
   ...
   mi->var.bits_per_pixel = mi->var.bits_per_pixel;
  if (ioctl(mi->fb, FBIOPUT_VSCREENINFO, &mi->var) < 0) {
      ALOGV("active fb swap failed");
  }
...
void fbdev_blank(LINUXFBDR_INTERNALP mi, int blank)
{
    int ret;

    ret = ioctl(mi->fb, FBIOBLANK, blank == 1 ? FB_BLANK_POWERDOWN : FB_BLANK_UNBLANK);
    if (ret < 0)
      ALOGV("ioctl(): blank");
}
/*
