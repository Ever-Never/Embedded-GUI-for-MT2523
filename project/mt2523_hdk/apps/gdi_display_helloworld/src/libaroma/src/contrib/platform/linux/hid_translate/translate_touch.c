...
#ifndef SYN_MT_BTN_TOUCH
#define SYN_MT_BTN_TOUCH 330
#endif
 ...
 */
static byte LINUXHIDRV_calibrate(LIBAROMA_HIDP me, LINUXHIDRV_POSP p, int * x,
		int * y) {
	...
	//	}
	//	/* calculation */
	//	*x = (p->x - p->xi.minimum) * (fb_width - 1) / (p->xi.maximum
	//			- p->xi.minimum);
	//	*y = (p->y - p->yi.minimum) * (fb_height - 1) / (p->yi.maximum
	//			- p->yi.minimum);
 ...
 */
byte LINUXHIDRV_translate_touch(LIBAROMA_HIDP me, LINUXHIDRV_DEVICEP dev,
	...
	if (ev->type == EV_KEY) {
		if (ev->code == 330) {
			switch (ev->value) {
				case DOWN:
				ALOGRT("Touch  ------------------->button down ");
				//	MT_TRACKING_IS_UNTOUCHED = 0;
				break;
				case UP:
				ALOGRT("Touch  -------------------> button up");
	...
	if (ev->type == EV_SYN) {
		...
		// ((dev->p.state & (LINUXHIDRV_POS_ST_LASTSYNC| LINUXHIDRV_POS_ST_RLS_NEXT)) && !MT_TRACKING_IS_UNTOUCHED)||
		if ((MT_TRACKING_IS_UNTOUCHED && TOUCH_RELEASE_NEXTSYN == 1)) {
			...
			if (!(dev->p.state & LINUXHIDRV_POS_ST_IS_VKEY)) {
				...
				if ((xd < dev->vks[dev->p.vk].w / 2)
						&& (yd < dev->vks[dev->p.vk].h / 2)) {
					/* it still on virtual key. set as up */
					key_ev.value = 0;
					/* log raw */
					ALOGRT("INDR VIRTUALKEY UP : [%i,%i] on %ix%ipx", dev->p.vk,
							key_ev.code, xd, yd);
				} else {
					/* log raw */
		...
		if (!(dev->p.state & LINUXHIDRV_POS_ST_DOWNED)) {
			...
			for (i = 0; i < dev->vkn; i++) {
				...
				if ((xd < dev->vks[i].w / 2) && (yd < dev->vks[i].h / 2)) {
					...
					key_ev.value = 1;
					/* log raw */
					ALOGRT("INDR VIRTUALKEY DOWN : [%i,%i] on %ix%ipx", i,
							key_ev.code, xd, yd);
					/* if on virtual key - send as keyboard event */
					return LINUXHIDRV_translate_keyboard(me, dev, dest_ev,
							&key_ev);
				}
			}
		...
		/* on touch move */
			...
			if ((dev->p.state & LINUXHIDRV_POS_ST_IS_VKEY)) {
				...
				ALOGRT(
						"RAW TOUCH STATUS - needed for cancel virtual key event");
	...
	/* reset last sync event */
	dev->p.state &= ~LINUXHIDRV_POS_ST_LASTSYNC;
	return_none: return LIBAROMA_HID_EV_RET_NONE;
#endif
