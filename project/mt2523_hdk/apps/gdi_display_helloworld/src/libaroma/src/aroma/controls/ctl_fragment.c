...
void _libaroma_ctl_fragment_measure(LIBAROMA_WINDOWP win)
{
  ...
  printf("_libaroma_ctl_fragment_measure  start \n");
  _VALIDATE_FRAGMENT();
  libaroma_mutex_lock(me->dmutex);
  win->x = 0;
  win->y = 0;
...
{
  printf("_libaroma_ctl_fragment_activate_win  start \n");
  _VALIDATE_FRAGMENT();
  LIBAROMA_MSG msg;
  if (!active)
  {
...
#pragma omp parallel for
  ...
  else
  {
    if (!win->active)
    {
      ...
      printf("_libaroma_ctl_fragment_activate_win \n");
      libaroma_wm_compose(
...
#pragma omp parallel for
  ...
  printf("_libaroma_ctl_fragment_activate_win  end \n");
}
...
byte _libaroma_ctl_fragment_window_updatebg(LIBAROMA_WINDOWP win)
{
  ...
  printf("_libaroma_ctl_fragment_window_updatebg  start \n");
  _VALIDATE_FRAGMENT(0);
  //libaroma_mutex_lock(me->dmutex);
  int w = win->w;
  int h = win->h;
  ...
  libaroma_canvas_setcolor(
      ...
      0xff);
  // libaroma_mutex_unlock(me->dmutex);
  printf("_libaroma_ctl_fragment_window_updatebg  end \n");
  return 1;
} /* End of _libaroma_ctl_fragment_window_sync */
    ...
    LIBAROMA_CANVASP c)
{
  ...
  printf("_libaroma_ctl_fragment_draw start\n");
...
#pragma omp parallel for
  ...
  me->redraw = 0;
  libaroma_mutex_unlock(me->mutex);
  printf("_libaroma_ctl_fragment_draw end\n");
...
byte _libaroma_ctl_fragment_thread(LIBAROMA_CONTROLP ctl)
{
  ...
  printf("_libaroma_ctl_fragment_thread start\n");
  /* internal check */
  _LIBAROMA_CTL_CHECK(
      _libaroma_ctl_fragment_handler, _LIBAROMA_CTL_FRAGMENTP, 0);
  printf("_libaroma_ctl_fragment_thread win_n %d  %d %d\n", me->win_n, me->win_pos, me->win_next_del_id);
  if ((me->win_n < 1) || (me->win_pos == -1))
  {
    return 0;
  }

  printf("_libaroma_ctl_fragment_thread  libaroma_mutex_lock start\n");
  libaroma_mutex_lock(me->mutex);
  printf("_libaroma_ctl_fragment_thread  libaroma_mutex_lock end\n");
  if (me->win_next_del_id != -1)
  {
    printf("libaroma_ctl_fragment_del_window_nomutex  start\n");
    libaroma_ctl_fragment_del_window_nomutex(ctl, me->win_next_del_id);
    printf("libaroma_ctl_fragment_del_window_nomutex  end\n");
    me->win_next_del_id = -1;
  }
  byte is_draw = me->redraw;
  {
    printf("draw child start \n");
    int j;
#ifdef LIBAROMA_CONFIG_OPENMP
#pragma omp parallel for
#endif
...
#pragma omp parallel for
          ...
          for (i = 0; i < win->childn; i++)
          {
            ...
            {
              printf("thread start\n");
              byte dd = c->handler->thread(c) ;
              printf("thread end\n");
              if (dd)
              {
                printf("thread need to draw child\n");
                if (libaroma_control_draw(c, 0))
                {
                  is_draw = 1;
                }
              }
              printf("thread no need to draw child\n");
            }
          }
        }
      }
  ...
  {
    printf("_libaroma_ctl_fragment_thread  transition_start transition_start %d win_pos_out %d\n", me->transition_start, me->win_pos_out);
  ...
  libaroma_mutex_unlock(me->mutex);
  printf("_libaroma_ctl_fragment_thread end\n");
  return is_draw;
    ...
    LIBAROMA_RECTP rect_end)
{
  _LIBAROMA_CTL_CHECK(
      _libaroma_ctl_fragment_handler, _LIBAROMA_CTL_FRAGMENTP, 0);
  printf("wait for transition start  me->win_pos_out %d \n", me->win_pos_out);
  /* wait for transition */
  while (me->win_pos_out != -1)
  {
    libaroma_sleep(16);
  }
  printf("wait for transition end \n");
  byte ret = 0;
  int i;
  int did = -1;
  libaroma_mutex_lock(me->mutex);
  ...
  printf("wait for transition %d %d  \n", did, me->win_pos);
  if (did != -1)
  {
    if (me->win_pos != did)
    {
      printf("wait  _libaroma_ctl_fragment_activate_win start \n");
      _libaroma_ctl_fragment_activate_win(win, 1);
      printf("wait  _libaroma_ctl_fragment_activate_win end \n");
      libaroma_sleep(120);
      printf("libaroma_sleep 120 transition %d %d  \n", did, me->win_pos);
      if (me->win_pos != -1)
      {
