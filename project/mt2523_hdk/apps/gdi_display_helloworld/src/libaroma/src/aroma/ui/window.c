 ...
 */
int libaroma_window_measure_calculate(int cv, int pos, int max, int is_size,
		int x) {
	if (is_size) {
		if (pos <= 0) {
			switch (pos) {
			case LIBAROMA_POS_HALF:
				return (max / 2) - x;
				break;
			case LIBAROMA_POS_1P3:
				return (max / 3) - x;
				break;
			case LIBAROMA_POS_2P3:
				return (max * 2 / 3) - x;
				break;
			case LIBAROMA_POS_1P4:
				return (max / 4) - x;
				break;
			case LIBAROMA_POS_3P4:
				return (max * 3 / 4) - x;
				break;
			case LIBAROMA_SIZE_FULL:
				return max;
				break;
			case LIBAROMA_SIZE_HALF:
				return max / 2;
				break;
			case LIBAROMA_SIZE_THIRD:
				return max / 3;
				break;
			case LIBAROMA_SIZE_QUARTER:
				return max / 4;
				break;
			default:
				return abs(pos);
...
byte libaroma_window_set_touch_handler(
  LIBAROMA_WINDOWP win, LIBAROMA_WINDOW_TOUCH_CB handler){
	  if(win != NULL)
	  {
		win->touch_handler =handler ;
		return 1;
	  }
	  return 0 ;
  }

void _libaroma_window_msg_thread(LIBAROMA_WINDOWP win ,LIBAROMA_MSGP msg) {
	if(win != NULL && msg != NULL)
	{
		dword ret  = libaroma_window_process_event(win, msg);
		if(win->touch_handler)
		{
			win->touch_handler(ret ,msg) ;
		}
	}
	return ;
}
...
LIBAROMA_WINDOWP libaroma_window(char * bg_theme_name, int x, int y, int w,
	...
	win->msg_thread = _libaroma_window_msg_thread ;
 ...
 */
byte libaroma_window_free(LIBAROMA_WINDOWP win) {
	__CHECK_WM(0);
	if (win == NULL) {
		return 0;
	}
	...
	if (win->parent == NULL) {
		...
		libaroma_window_process_event(
				win,
				libaroma_wm_compose(&_msg, LIBAROMA_MSG_WIN_INACTIVE, NULL, 0,
						0));
	}
 ...
 */
byte libaroma_window_resize(LIBAROMA_WINDOWP win, int x, int y, int w, int h) {
	if (!win) {
		return 0;
	}
	if (win->parent != NULL) {
 ...
 */
byte libaroma_window_del(LIBAROMA_WINDOWP win, LIBAROMA_CONTROLP ctl) {
	__CHECK_WM(0);
	if (ctl == NULL) {
	...
	LIBAROMA_CONTROLP * newchilds = (LIBAROMA_CONTROLP *) malloc(
			sizeof(LIBAROMA_CONTROLP) * (win->childn - 1));
	if (!newchilds) {
...
byte libaroma_window_measure(LIBAROMA_WINDOWP win, LIBAROMA_CONTROLP ctl) {
	if (win && ctl) {
		...
		ctl->x = libaroma_window_measure_calculate(ctl->x, ctl->rx, win->w, 0,
				0);
		ctl->y = libaroma_window_measure_calculate(ctl->y, ctl->ry, win->h, 0,
				0);
		ctl->w = libaroma_window_measure_calculate(ctl->w, ctl->rw, win->w, 1,
				ctl->x);
		ctl->h = libaroma_window_measure_calculate(ctl->h, ctl->rh, win->h, 1,
				ctl->y);
		...
		if (ctl->handler->message) {
			...
			ctl->handler->message(
					ctl,
					libaroma_wm_compose(&_msg, LIBAROMA_MSG_WIN_MEASURED, NULL,
							0, 0));
			return 1;
		}
	}
 ...
 */
LIBAROMA_CONTROLP libaroma_window_getid(LIBAROMA_WINDOWP win, word id) {
	__CHECK_WM(NULL);
	if (win == NULL) {
 ...
 */
LIBAROMA_CONTROLP libaroma_window_setfocus(LIBAROMA_WINDOWP win,
		LIBAROMA_CONTROLP ctl) {
	if (win == NULL) {
 ...
 */
byte libaroma_window_anishow(LIBAROMA_WINDOWP win, byte animation, int duration) {
	__CHECK_WM(0);
	if (!win) {
		return 0;
	}
	...
	if (libaroma_wm_set_active_window(win)) {
		...
		while ((delta = libaroma_tick() - start) < duration) {
			...
			case LIBAROMA_WINDOW_SHOW_ANIMATION_PAGE_BOTTOM: {
				...
				if (h > 0) {
					if (animation
							== LIBAROMA_WINDOW_SHOW_ANIMATION_SLIDE_BOTTOM) {
						if (h < win->h) {
							libaroma_draw_ex(
									wmc,
									back,
		...
		/* cleanup */libaroma_canvas_free(back);
	...
	if (retval) {
		...
		libaroma_window_process_event(
				win,
				libaroma_wm_compose(&_msg, LIBAROMA_MSG_WIN_ACTIVE, NULL, 10, 0));
	}
	return retval;
 ...
 */
void libaroma_window_calculate_pos(LIBAROMA_WINDOWP win, LIBAROMA_CONTROLP ctl,
		int * x, int * y) {
	if (win != NULL) {
		*x -= win->x;
		*y -= win->y;
 ...
 */
void libaroma_window_calculate_pos_abs(LIBAROMA_WINDOWP win,
		LIBAROMA_CONTROLP ctl, int * x, int * y) {
	if (ctl != NULL) {
		*x -= ctl->x;
		*y -= ctl->y;
		win = ctl->window;
...
byte libaroma_window_post_command(dword cmd) {
	return libaroma_msg_post(LIBAROMA_MSG_WIN_DIRECTMSG, 0, 0, (int) cmd, 0,
			NULL);
 ...
 */
byte libaroma_window_post_command_ex(dword cmd, byte state, int key, int y,
		voidp d) {
	return libaroma_msg_post(LIBAROMA_MSG_WIN_DIRECTMSG, state, key, (int) cmd,
			y, d);
} /* End of libaroma_window_post_command */
