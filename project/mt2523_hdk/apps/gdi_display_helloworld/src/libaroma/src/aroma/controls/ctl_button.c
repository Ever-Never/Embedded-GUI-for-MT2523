...
static LIBAROMA_CONTROL_HANDLER _libaroma_ctl_button_handler = { message
		:_libaroma_ctl_button_msg, draw:_libaroma_ctl_button_draw, focus:NULL,
		destroy:_libaroma_ctl_button_destroy, thread
...
int libaroma_ctl_button_width(const char * text) {
	LIBAROMA_TEXT textp = libaroma_text(text, 0, libaroma_fb()->w,
	LIBAROMA_FONT(0,4) |
...
void _libaroma_ctl_button_req_internal_draw(LIBAROMA_CONTROLP ctl) {
	...
	_libaroma_ctl_button_req_internal_draw_thread(ctl);
}
