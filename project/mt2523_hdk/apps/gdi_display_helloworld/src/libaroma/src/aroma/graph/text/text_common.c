 ...
 */
byte _libaroma_text_concat_ex(char ** dst, const char * src, byte is_pre) {
	if (src == NULL) {
		return 0;
 ...
 */
byte _libaroma_text_compare(const char * str, const char * find) {
	int sl = strlen(str);
	int fl = strlen(find);
	if (fl > sl) {
		return 0;
 ...
 */
char * _libaroma_text_parse_strdup(const char * src) {
	if (src == NULL) {
		return NULL;
	}
	int slen = strlen(src);
 ...
 */
void _libaroma_text_change_current(_LIBAROMA_TEXTCHUNKP chunk, const char * val) {
 ...
 */
static byte _libaroma_text_direction(uchar c, byte prev_rtl) {
