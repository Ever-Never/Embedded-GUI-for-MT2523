...
#define LIBAROMA_CANVAS_ARRAY  0x04
 ...
 */
LIBAROMA_CANVASP libaroma_canvas_new_array(
    int w,
    int h,
    byte useAlpha,
    byte hiColor,
...
LIBAROMA_CANVASP libaroma_canvas_new_ex(
  ...
  c->s        = w * h;
  //c->data     = (wordp) calloc(c->s,2);
  c->data     = (wordp) pvPortMalloc(c->s*2);
...
void libaroma_canvas_free_ex1(
  ...
  else if (cv->flags & LIBAROMA_CANVAS_ARRAY) {
    goto  freealpha;
  }
