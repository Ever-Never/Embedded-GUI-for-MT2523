...
void libaroma_dialog_updatebg(LIBAROMA_WINDOWP win, LIBAROMA_CANVASP c) {
	...
	/* draw bg */libaroma_draw(c,cdata->bg,0,0,0);
	/*
	...
	if (cdata->flags & LIBAROMA_DIALOG_WITH_SHADOW) {
		libaroma_draw_shadow(c, cdata->x + libaroma_dp(2),
				cdata->y + libaroma_dp(24), cdata->w - libaroma_dp(4),
				cdata->h - libaroma_dp(26), libaroma_dp(24), libaroma_dp(32),
				0x40, 0);
	}

	word bgcolor = libaroma_colorget(NULL, win)->dialog_bg;

	/* draw dialog */libaroma_gradient(c,
			cdata->x, cdata->y,
			cdata->w, cdata->h,
			bgcolor,bgcolor,
			libaroma_dp(2),
 ...
 */
LIBAROMA_WINDOWP libaroma_dialog_window(int w, int h, byte flags,
	...
	LIBAROMA_DIALOG_DATAP cdata = (LIBAROMA_DIALOG_DATAP) calloc(
			sizeof(LIBAROMA_DIALOG_DATA), 1);
	if (!cdata) {
	...
	LIBAROMA_CANVASP wmcv = libaroma_wm_canvas(0, 0, libaroma_wm()->w,
			libaroma_wm()->h);
	...
	/* create window */
	LIBAROMA_WINDOWP win = libaroma_window(NULL, 0, 0, libaroma_wm()->w,
			libaroma_wm()->h);
 ...
 */
byte libaroma_dialog_free(LIBAROMA_WINDOWP win) {
	if (win) {
		if (win->client_data) {
			LIBAROMA_DIALOG_DATAP cdata =
					(LIBAROMA_DIALOG_DATAP) win->client_data;
...
int libaroma_dialog_confirm(const char * title, const char * message,
		const char * button1, const char * button2,
		LIBAROMA_COLORSETP colorset, byte flags) {
	...
	if (title) {
		title_h = libaroma_ctl_label_height(
				...
				0,
				6,
				LIBAROMA_TEXT_LEFT | LIBAROMA_TEXT_FIXED_INDENT
						| LIBAROMA_TEXT_NOHR, 100);
	}
	int msg_h = 0;
	if (message) {
		msg_h = libaroma_ctl_label_height(
				...
				4,
				LIBAROMA_TEXT_LEFT | LIBAROMA_TEXT_FIXED_INDENT
						| LIBAROMA_TEXT_NOHR, 120);
	}
	int dialog_h = libaroma_dp(120) + title_h + msg_h;

	LIBAROMA_WINDOWP win = libaroma_dialog_window(dialog_w, dialog_h, flags,
			colorset);
	...
	if (title) {
		libaroma_ctl_label_valign(
				...
				libaroma_px(text_w),
				libaroma_px(title_h + 10),
				title_color,
				0,
				6,
				LIBAROMA_TEXT_LEFT | LIBAROMA_TEXT_FIXED_INDENT
						| LIBAROMA_TEXT_NOHR, 100, 10);
	}
	if (message) {
		libaroma_ctl_label_valign(
				win,
				...
				libaroma_px(text_w),
				libaroma_px(msg_h + 10),
				msg_color,
				0,
				4,
				LIBAROMA_TEXT_LEFT | LIBAROMA_TEXT_FIXED_INDENT
						| LIBAROMA_TEXT_NOHR, 120, 10);
	}
	...
	if (button1) {
		...
		libaroma_ctl_button(win, 1, button1_x, button_y,
				libaroma_px(button1_w), 36, button1, button_style, btncolor);

		if (button2) {
			int button2_w = libaroma_ctl_button_width(button2);
			int button2_x = button1_x - libaroma_px(button2_w);

			libaroma_ctl_button(win, 2, button2_x, button_y,
					libaroma_px(button2_w), 36, button2, button_style, btncolor);
		}
	}
	...
	do {
		...
		if (msg.msg == LIBAROMA_MSG_KEY_BACK) {
			...
			if (flags & LIBAROMA_DIALOG_CANCELABLE) {
				if (msg.state == 1) {
					if ((msg.x < cdata->x) || (msg.y < cdata->y) || (msg.y
							> cdata->y + cdata->h) || (msg.x > cdata->x
							+ cdata->w)) {
...
byte _libaroma_dialog_list_simple_option_cb(LIBAROMA_CONTROLP ctl,
		LIBAROMA_CTL_LIST_ITEMP item, int id, byte checked, voidp data,
		byte state) {
...
int libaroma_dialog_list(const char * title, const char * button1,
	...
	LIBAROMA_CONTROLP listc = libaroma_ctl_list(win, 999,
			libaroma_px(cdata->x),
			libaroma_px(cdata->y) + 44 + libaroma_px(title_h),
			libaroma_px(cdata->w), 10, 0, 0,
	...
	for (u = 0; u < 24; u++) {
		...
		LIBAROMA_CTL_LIST_ITEMP itm = libaroma_listitem_check(
				...
				LIBAROMA_LISTITEM_CHECK_LEFT_CONTROL
						| LIBAROMA_LISTITEM_CHECK_OPTION, -1);

		libaroma_listitem_check_set_cb(listc, itm,
				_libaroma_dialog_list_simple_option_cb, (voidp) &selitem);

		if (u == 15) {
			libaroma_listitem_set_selected(listc, itm, 1);
		}
	}
	...
	libaroma_control_resize(listc, libaroma_px(cdata->x),
			libaroma_px(cdata->y) + 44 + libaroma_px(title_h),
			libaroma_px(cdata->w), libaroma_px(msg_h + 5));
	...
	if (selitem) {
		libaroma_ctl_list_scroll_to_item(listc, selitem, 0);
	}
