...
static _LIBAROMA_FONT_SLOT_CACHEP _libaroma_font_hb_load_glyph(
		_LIBAROMA_FONT_FACEP aface, hb_codepoint_t glyph) {
 ...
 */
/* glyph */
static hb_bool_t _libaroma_font_hb_glyph_func(hb_font_t * font,
		void * font_data, hb_codepoint_t unicode,
...
/* glyph_h_advance */
static hb_position_t _libaroma_font_hb_glyph_h_advance_func(hb_font_t * font,
...
/* glyph_v_advance */
static hb_position_t _libaroma_font_hb_glyph_v_advance_func(hb_font_t * font,
		void * font_data, hb_codepoint_t glyph, void * user_data) {
	_LIBAROMA_FONT_HB_FTFACE();
	FT_Fixed v;
	_libaroma_font_lock(1);
	if (FT_Get_Advance(ft_face, glyph,
			_LIBAROMA_FONT_LOAD_GLYPH_FLAG | FT_LOAD_VERTICAL_LAYOUT, &v)) {
		_libaroma_font_lock(0);
		return 0;
	}
	_libaroma_font_lock(0);
...
/* glyph_h_origin */
static hb_bool_t _libaroma_font_hb_glyph_h_origin_func(hb_font_t * font,
		void * font_data, hb_codepoint_t glyph, hb_position_t * x,
		hb_position_t * y, void * user_data) {
	return true;
}
/* glyph_v_origin */
static hb_bool_t _libaroma_font_hb_glyph_v_origin_func(hb_font_t * font,
		void * font_data, hb_codepoint_t glyph, hb_position_t * x,
		hb_position_t * y, void * user_data) {
...
/* glyph_h_kerning */
static hb_position_t _libaroma_font_hb_glyph_h_kerning_func(hb_font_t * font,
		void * font_data, hb_codepoint_t left_glyph,
		hb_codepoint_t right_glyph, void * user_data) {
	_LIBAROMA_FONT_HB_FTFACE();
	FT_Vector kerningv;
	unsigned int x_ppem;
	unsigned int y_ppem;
...
/* glyph_v_kerning */
static hb_position_t _libaroma_font_hb_glyph_v_kerning_func(hb_font_t * font,
		void * font_data, hb_codepoint_t top_glyph,
		hb_codepoint_t bottom_glyph, void * user_data) {
	return 0;
}
/* glyph_extents */
static hb_bool_t _libaroma_font_hb_glyph_extents_func(hb_font_t * font,
		void * font_data, hb_codepoint_t glyph, hb_glyph_extents_t * extents,
		void * user_data) {
	_LIBAROMA_FONT_HB_LOADFACE();
	if (!loaded_glyph) {
		return false;
...
/* glyph_contour_point */
static hb_bool_t _libaroma_font_hb_glyph_contour_point_func(hb_font_t * font,
		void * font_data, hb_codepoint_t glyph, unsigned int point_index,
...
/* glyph_name */
static hb_bool_t _libaroma_font_hb_glyph_name_func(hb_font_t * font,
		void * font_data, hb_codepoint_t glyph, char * name, unsigned int size,
		void * user_data) {
	_LIBAROMA_FONT_HB_FTFACE();
	_libaroma_font_lock(1);
...
/* glyph_from_name */
static hb_bool_t _libaroma_font_hb_glyph_from_name_func(hb_font_t * font,
		void * font_data, const char * name, int len, hb_codepoint_t * glyph,
		void * user_data) {
	_LIBAROMA_FONT_HB_FTFACE();
	if (len < 0) {
		_libaroma_font_lock(1);
 ...
 */
static hb_blob_t * _libaroma_font_hb_ref_table(hb_face_t * face, hb_tag_t tag,
		void * font_data) {
	_LIBAROMA_FONT_HB_FTFACE();
	FT_ULong length = 0;
	FT_Byte * buffer;
 ...
 */
void _libaroma_font_hb_update_scale(byte fontid) {
	...
	if (aface->hb_font) {
		...
		/* set ppem */
		hb_font_set_ppem(aface->hb_font, aface->face->size->metrics.x_ppem,
 ...
 */
void _libaroma_font_hb_free(byte fontid) {
	/* Free Harfbuzz Font */
 ...
 */
hb_font_t * _libaroma_font_hb_init(byte fontid) {
	_LIBAROMA_FONT_FACEP aface =
			(_LIBAROMA_FONT_FACEP) (&_libaroma_font_faces[fontid]);
	FT_Face ft_face = aface->face;
	/* init harfbuzz face */
	...
	if (ft_face->stream->read == NULL) {
		hb_blob_t * blob;
		blob
				= hb_blob_create((const char *) ft_face->stream->base,
		...
		hb_face = hb_face_create_for_tables(_libaroma_font_hb_ref_table,
				(void *) aface, NULL);
	}
	/* set index */
	...
	hb_font_set_funcs(hb_font, _libaroma_font_hb_init_callbacks(),
			(void *) aface,
