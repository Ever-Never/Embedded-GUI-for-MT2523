...
byte _libaroma_wm_workspace_canvas_update()
{
	...
	{
		libaroma_draw_ex(_libaroma_wm->workspace_bg, libaroma_fb()->canvas, 0,
						 0, _libaroma_wm->x, _libaroma_wm->y, _libaroma_wm->w,
...
byte _libaroma_wm_workspace_canvas()
{
	...
	_libaroma_wm->workspace_bg = libaroma_canvas(_libaroma_wm->w,
												 _libaroma_wm->h);
...
byte libaroma_wm_init()
{
	...
	//	libaroma_thread_create_rtos(&_libaroma_wm_message_thread_var,
	//							   _libaroma_wm_message_thread, NULL,"windowmanager_message_thread");

	/* start ui thread */
	libaroma_thread_create_rtos(&_libaroma_wm_ui_thread_var,
								_libaroma_wm_ui_thread, NULL, "windowmanger_ui_thread");

	/* high priority thread */
	libaroma_thread_set_hiprio(_libaroma_wm_ui_thread_var);
	vTaskSuspend(_libaroma_wm_ui_thread_var);

	/* map queue */
	message_queue_map(UI_SET_UPDATE_FREQUECY, libaroma_get_global_queue()->msg_queue);
	message_queue_map(UI_UPDATE, libaroma_get_global_queue()->msg_queue);
	message_queue_map(UI_TP_UPLOAD, libaroma_get_global_queue()->msg_queue);
	message_queue_map(UI_KEY_UPLOAD, libaroma_get_global_queue()->msg_queue);
	//vTaskSuspend(_libaroma_wm_message_thread_var) ;
	return 1;
...
void libaroma_wm_syncarea()
{
	...
	else
	{
		...
		if (mx > cx)
		{
			libaroma_wm_sync(_libaroma_wm->sync_x, _libaroma_wm->sync_y, dw,
							 dh);
		}
		else
		{
...
byte libaroma_wm_set_workspace(int x, int y, int w, int h)
{
	...
	if (_libaroma_wm->active_window != NULL)
	{
		/* send refresh event */
		LIBAROMA_MSG _msg;
		libaroma_window_process_event(_libaroma_wm->active_window,
									  libaroma_wm_compose(&_msg, LIBAROMA_MSG_WIN_RESIZE, NULL, 0,
														  0));
	}
	return 1;
...
byte libaroma_wm_getmessage(LIBAROMA_MSGP msg)
{
	...
	if (xQueueReceive(libaroma_get_global_queue()->msg_queue, msg, pdMS_TO_TICKS(5)))
	{
	...
	while (_libaroma_wm->client_started)
	{
		...
		if (_libaroma_wm->client_started)
		{
			...
			if (!_libaroma_wm_onprocessing)
			{
				...
				if (-1 != MessageRecvTimeout(libaroma_get_global_queue()->msg_queue, &msg, &msg_data, delay_ms))
				{

					ALOGD("=============== receive msg id ====================== %d", msg);
					switch (msg)
					{
						case UI_SET_UPDATE_FREQUECY:
						{
							ALOGD("===============set ui update delay frequency ====================== %ld" ,msg_data.sent);
							delay_ms = (int)msg_data.sent;
							break;
						}
						case UI_KEY_UPLOAD:
						case UI_TP_UPLOAD:
						{
							LIBAROMA_MSGP libaroma_msgp = &msg_data;
							if (_libaroma_wm->active_window != NULL && _libaroma_wm->active_window->msg_thread != NULL)
							{
								ALOGD("=================>>>>> msg_thread <<<<<<===========");
								_libaroma_wm->active_window->msg_thread(_libaroma_wm->active_window, libaroma_msgp);
							}
							break;
						}
					}
...
byte libaroma_wm_client_start()
{
	...
	if (!_libaroma_wm->client_started)
	{
		...
		vTaskResume(_libaroma_wm_ui_thread_var);
		//vTaskResume(_libaroma_wm_message_thread_var) ;
...
byte libaroma_wm_draw_themep(LIBAROMA_CANVASP dest, LIBAROMA_WM_THEMEP th,
							 int dx, int dy, int dw, int dh, LIBAROMA_PNG9_PADP padding)
{
	if (_libaroma_wm == NULL)
	{
		ALOGW("window manager uninitialized");
	...
	if ((th->canvas->w == dw) && (th->canvas->h == dh))
	{
		...
		{
			return libaroma_draw_scale_nearest(dest, th->canvas, dx, dy, dw, dh,
...
byte libaroma_wm_set_active_window(LIBAROMA_WINDOWP win)
{
	...
	ALOGD("libaroma_wm_set_active_window");
	if (_libaroma_wm == NULL)
	{
		ALOGW("window manager uninitialized");
		return 0;
	...
	if (_libaroma_wm->active_window != NULL)
	{
		...
		libaroma_window_process_event(cwin,
									  libaroma_wm_compose(&_msg, LIBAROMA_MSG_WIN_INACTIVE,
														  (voidp)win, 0, 0));
	}
	...
	else
	{
		...
		{
			/* start message client if not started yet */ libaroma_mutex_unlock(
