 ...
 */
void _libaroma_jpeg_mem_skip_input_data(j_decompress_ptr cinfo, long num_bytes) {
	_LIBAROMA_JPEG_MEM_MGRP src = (_LIBAROMA_JPEG_MEM_MGRP) cinfo->src;
	if (num_bytes > 0) {
...
void _libaroma_jpeg_mem_src(j_decompress_ptr cinfo, const JOCTET * buffer,
	...
	if (cinfo->src == NULL) {
		cinfo->src = (struct jpeg_source_mgr *) (*cinfo->mem->alloc_small)(
				...
				SIZEOF(__LIBAROMA_JPEG_MEM_MGR));
	}
...
LIBAROMA_CANVASP libaroma_jpeg_ex(LIBAROMA_STREAMP stream, byte freeStream,
	...
	/* init */jpeg_create_decompress(&cinfo);
	...
	if (hicolor) {
		while (cinfo.output_scanline < cinfo.output_height) {
			...
			for (x = 0, z = row_sz; x < z; x += 3) {
				*line++ = libaroma_rgb(row_data[x], row_data[x + 1],
						row_data[x + 2]);
				*hicl++ = libaroma_color_left(row_data[x], row_data[x + 1],
						row_data[x + 2]);
			}
			y++;
		}
 ...
 */
byte libaroma_jpeg_draw(LIBAROMA_STREAMP stream, byte freeStream,
		LIBAROMA_CANVASP cv, int dx, int dy, int dw, int dh, byte dither) {
	...
	if ((cinfo.output_width == vdw) && (cinfo.output_height == vdh)) {
		...
		free(row_data);
		libaroma_draw_scale_nearest(cv, cvt, dx, dy, dw, dh, 0, 0, cvt->w,
				cvt->h);
