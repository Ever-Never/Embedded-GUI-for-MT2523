 ...
 */
byte libaroma_control_resize(LIBAROMA_CONTROLP ctl, int x, int y, int w, int h) {
	if (!ctl) {
		return 0;
	}
	ctl->rx = x;
 ...
 */
byte libaroma_control_erasebg(LIBAROMA_CONTROLP ctl, LIBAROMA_CANVASP canvas) {
	if (ctl == NULL) {
	...
	if (win->handler != NULL) {
		if (win->handler->control_erasebg != NULL) {
			return win->handler->control_erasebg(win, ctl, canvas);
		}
	}
	if (win->bg == NULL) {
...
byte libaroma_control_isvisible(LIBAROMA_CONTROLP ctl) {
	...
	if (win->handler != NULL) {
		if (win->handler->control_isvisible != NULL) {
			return win->handler->control_isvisible(win, ctl);
		}
	}
	int sx = ctl->x;
	int sy = ctl->y;
 ...
 */
byte libaroma_control_free(LIBAROMA_CONTROLP ctl) {
	if (!ctl) {
		return 0;
	}
