...
static LIBAROMA_CONTROL_HANDLER _libaroma_ctl_progress_handler = { message
		:_libaroma_ctl_progress_msg, draw:_libaroma_ctl_progress_draw, focus
		:NULL, destroy:_libaroma_ctl_progress_destroy, thread
...
byte _libaroma_ctl_progress_thread(LIBAROMA_CONTROLP ctl) {
	...
	byte res = 0;
	// printf("_libaroma_ctl_progress_thread=======================> \r\n") ;
	if ((me->type & 3) != LIBAROMA_CTL_PROGRESS_DETERMINATE) {
		if (me->type & LIBAROMA_CTL_PROGRESS_CIRCULAR) {
			...
			float distance = (float) cos((me->growtime / 600.0 + 1) * __PI)
					/ 2.0 + 0.5;
			if (me->currx) {
			...
			me->state = libaroma_control_state(me->tick,
					_LIBAROMA_CTL_PROGRESS_BEZIER_TIMING*4);
		...
		me->state = libaroma_control_state(me->tick,
				_LIBAROMA_CTL_PROGRESS_BEZIER_TIMING);
...
void _libaroma_ctl_progress_draw(LIBAROMA_CONTROLP ctl, LIBAROMA_CANVASP c) {
	/* internal check */
	// printf("_libaroma_ctl_progress_draw=======================> \r\n") ;
	_LIBAROMA_CTL_CHECK(_libaroma_ctl_progress_handler, _LIBAROMA_CTL_PROGRESSP,
			);
	...
	if (me->type & LIBAROMA_CTL_PROGRESS_CIRCULAR) {
		...
		libaroma_draw_arc(c, c->w >> 1, c->h >> 1, sz, sz, width, from,
				from + length, libaroma_colorget(ctl, NULL)->primary, 0xff, 0,
				0.5);
	} else {
...
LIBAROMA_CONTROLP libaroma_ctl_progress(LIBAROMA_WINDOWP win, word id, int x,
		int y, int w, int h, byte type, int max, int value) {
	/* init internal data */
