 ...
 */
byte libaroma_msg_post_hid(byte msg, byte state, int key, int x, int y); /* End of libaroma_msg_post_hid */
...
byte libaroma_msg_post(byte msg, byte state, int key, int x, int y, voidp d)
{
	...
	/* mutex lock */
	libaroma_cond_lock(&_libaroma_msgqueue_mutex);
	MessageSend(UI_TP_UPLOAD, &_msg, sizeof(LIBAROMA_MSG), MESSAGE_IS_POINTER);
	/* mutex unlock */
	libaroma_cond_unlock(&_libaroma_msgqueue_mutex);
	return 1;
...
byte libaroma_msg_post_hid(byte msg, byte state, int key, int x, int y)
{
	...
	_msg.sent = libaroma_tick();
	/* mutex lock */
	 ALOGI("=====================>>>>>>>>> MessageSend lock start ") ;
	libaroma_cond_lock(&_libaroma_msgqueue_mutex);
	 ALOGI("=====================>>>>>>>>> MessageSend start ") ;
	 MessageSend(UI_TP_UPLOAD, &_msg, sizeof(LIBAROMA_MSG), MESSAGE_IS_POINTER);
	 ALOGI("=====================>>>>>>>>> MessageSend end ") ;
	libaroma_cond_unlock(&_libaroma_msgqueue_mutex);
	/* return status */
	return 1;
...
{
	// /* mutex lock */ libaroma_cond_lock(&_libaroma_msgqueue_mutex);
	// /* wait until data available in queue */
	// while ((_libaroma_msgqueue->queue->n + _libaroma_msgqueue->input->n) < 1)
	// {
	// 	libaroma_cond_unlock(&_libaroma_msgqueue_mutex);
	// 	/* wait for signal */ libaroma_cond_wait(&_libaroma_msgqueue_cond, &_libaroma_msgqueue_mutex);
	// 	libaroma_cond_lock(&_libaroma_msgqueue_mutex);
	...
	// 	free(shift_event);
	// }
	// /* unlock mutex */ libaroma_cond_unlock(&_libaroma_msgqueue_mutex);
	return 0;
} /* End of libaroma_msg */
