...
/* window handler */
byte _libaroma_window_layer_sync(LIBAROMA_WINDOWP win, int x, int y, int w,
		int h);
...
byte _libaroma_window_layer_message_hooker(LIBAROMA_WINDOWP win,
		LIBAROMA_MSGP msg, dwordp retval);
byte _libaroma_window_layer_updatedc(LIBAROMA_WINDOWP win);
static LIBAROMA_WINDOW_HANDLER _libaroma_window_layer_handler = { .prefree=NULL,
...
LIBAROMA_CANVASP _libaroma_window_layer_control_draw_begin(
	...
	c = libaroma_canvas_area(dc, cctl->x, cctl->y, cctl->w, cctl->h);
	libaroma_mutex_unlock(me->mutex);
	return c;
...
byte _libaroma_window_layer_message_hooker(LIBAROMA_WINDOWP win,
	...
	if (win->active == 1) {
		if (me->sidebar_showed == 2) {
			if (me->sidebar) {
				if (me->sidebar->handler->message_hooker(me->sidebar, msg,
						retval)) {
					return 1;
				}
			}
			...
			case LIBAROMA_MSG_TOUCH: {
				...
				if (msg->state == LIBAROMA_HID_EV_STATE_DOWN) {
					...
					if (msg->state == LIBAROMA_HID_EV_STATE_MOVE) {
						...
						libaroma_mutex_lock(me->mutex);
						me->sidebar_velocity = ((int) (libaroma_fling_up(
								&me->fling, x) * 1.3) >> 8);
...
byte libaroma_window_layer_init(LIBAROMA_WINDOWP win) {
	...
	_LIBAROMA_WINDOW_LAYERP me = (_LIBAROMA_WINDOW_LAYERP) calloc(
			sizeof(_LIBAROMA_WINDOW_LAYER), 1);
	if (!me) {
...
byte _libaroma_window_sidebar_sync(LIBAROMA_WINDOWP win, int x, int y, int w,
		int h);
byte _libaroma_window_sidebar_message_hooker(LIBAROMA_WINDOWP win,
		LIBAROMA_MSGP msg, dwordp retval);
static LIBAROMA_WINDOW_HANDLER _libaroma_window_sidebar_handler = { .prefree=NULL,
.postfree=NULL,
.updatebg=NULL,
...
byte _libaroma_window_sidebar_message_hooker(LIBAROMA_WINDOWP win,
				...
				if ((abs(move_sz_y) >= scrdp) && (abs(move_sz_y)
					...
					if (me->pretouched) {
						if (me->pretouched->handler->message) {
							...
							me->pretouched = NULL;
							win->touched->handler->message(win->touched,
									&me->pretouched_msg);
							win->touched->handler->message(win->touched, msg);
						} else {
							me->pretouched = NULL;
						}
						me->client_touch_start = 0;
					...
					me->touch_y = y;
					libaroma_mutex_unlock(me->mutex);
					_libaroma_window_layer_set_sidebar_pos(win->parent,
							win->w - 1);
					libaroma_mutex_lock(me->mutex);
				}
			}
	...
	case LIBAROMA_MSG_WIN_MEASURED: {
		int target_w = libaroma_window_usedp(2) ? win->rw
				: libaroma_dp(win->rw);
		target_w = libaroma_window_measure_calculate(target_w, win->rw,
				win->parent->w, 1, 0);
	...
	end_message: libaroma_mutex_unlock(me->mutex);
	return return_state;
...
LIBAROMA_WINDOWP libaroma_window_sidebar(LIBAROMA_WINDOWP win, int width) {
	...
	libaroma_mutex_lock(me->mutex);
	LIBAROMA_WINDOWP sidebar = (LIBAROMA_WINDOWP) calloc(
			sizeof(LIBAROMA_WINDOW), 1);
	if (!sidebar) {
		libaroma_mutex_unlock(me->mutex);
 ...
 */
byte libaroma_window_sidebar_onslide(LIBAROMA_WINDOWP win,
		LIBAROMA_WINDOW_SIDEBAR_SLIDE_CB cb) {
	_LIBAROMA_WINDOW_LAYERP me = _libaroma_window_layer_check(win->parent);
	if (!me) {
		return 0;
	}
