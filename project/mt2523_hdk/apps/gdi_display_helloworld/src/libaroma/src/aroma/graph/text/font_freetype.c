 ...
 */
byte libaroma_font_set_size(byte fontid, short size, byte lock) {
	ALOGV("libaroma_font_set_size fontid(%i)>=%i", fontid,
	size);
	if (fontid >= _LIBAROMA_FONT_MAX_FACE) {
		ALOGW("libaroma_font_set_size fontid(%i)>=%i", fontid,
 ...
 */
_LIBAROMA_FONT_FACEP libaroma_font_get_face(byte fontid) {
	if (fontid >= _LIBAROMA_FONT_MAX_FACE) {
		ALOGW("libaroma_font_get_face fontid(%i)>=%i", fontid,
				_LIBAROMA_FONT_MAX_FACE);
		return NULL;
	}
 ...
 */
int libaroma_font_set_ucs2(FT_Face ftf) {
	int i;
 ...
 * Descriptions: get font face
 */
FT_Face libaroma_font_get(byte fontid, byte nofallback) {
	if (fontid >= _LIBAROMA_FONT_MAX_FACE) {
		ALOGW("libaroma_font_get fontid(%i)>=%i", fontid,
				_LIBAROMA_FONT_MAX_FACE);
		return 0;
	}
 ...
 */
int libaroma_font_glyph_get_id(int c, byte fontid, byte * fontid_avail) {
	*fontid_avail = 0;
	/* utf 8 flag */
	if (c == 0xfeff) {
		return 0;
 ...
 */
LIBAROMA_GLYPH libaroma_font_glyph(int c, byte fontid, byte size) {
	/* UTF 8 Flag */
	if (c == 0xfeff) {
		return NULL;
	}
	/* 0 - 9 */
	if (fontid >= _LIBAROMA_FONT_MAX_FACE) {
		ALOGW("libaroma_font_glyph fontid(%i)>=%i", fontid,
				_LIBAROMA_FONT_MAX_FACE);
		return NULL;
	}
	...
	_libaroma_font_lock(1);
	_LIBAROMA_FONT_SLOT_CACHEP tmp_glyph =
			(_LIBAROMA_FONT_SLOT_CACHEP) libaroma_iarray_get(
					_libaroma_font_faces[fontid].cache, cache_id);
	...
	if (FT_Load_Glyph(face, c, _LIBAROMA_FONT_LOAD_GLYPH_FLAG) == 0) {
		...
		/* ready to return */
		libaroma_iarray_set(_libaroma_font_faces[fontid].cache, cache_id,
				(voidp) &slot, sizeof(_LIBAROMA_FONT_SLOT_CACHE), 1);
		// _libaroma_font_faces[fontid].last_cache
		tmp_glyph = libaroma_iarray_get(_libaroma_font_faces[fontid].cache,
				cache_id);
		_libaroma_font_lock(0);
 ...
 */
void libaroma_font_freecache_cb(void * val) {
 ...
 */
byte libaroma_font(byte fontid, LIBAROMA_STREAMP stream) {
	if (!stream) {
	...
	/* thread safe */
	_libaroma_font_lock(1);
	ALOGV("font loaded %ibytes (%s) start ....", stream->size, stream->uri);
	/* load face */
	FT_Face tmp_face;
	if (FT_New_Memory_Face(_libaroma_font_instance, stream->data, stream->size,
			0, &tmp_face) == 0) {
		/* set default face size */
		ALOGV("set default face size");
		int def_size = libaroma_font_size_px(2);

		if (FT_Set_Pixel_Sizes(tmp_face, 0, def_size) == 0) {
			/* save it */
			libaroma_font_free(fontid);
		ALOGV("libaroma_font_free end ......");
			...
			_libaroma_font_faces[fontid].cache = libaroma_iarray(
 ...
 */
byte libaroma_font_free(byte fontid) {
	if (fontid >= _LIBAROMA_FONT_MAX_FACE) {
		ALOGW("libaroma_font_free fontid(%i)>=%i", fontid,
				_LIBAROMA_FONT_MAX_FACE);
		return 0;
	}
	//_libaroma_font_lock(1);
 ...
 */
short libaroma_font_glyph_width(LIBAROMA_GLYPH aglyph_param) {
 ...
 */
short libaroma_font_glyph_height(LIBAROMA_GLYPH aglyph_param) {
	_LIBAROMA_FONT_SLOT_CACHEP aglyph =
			(_LIBAROMA_FONT_SLOT_CACHEP) aglyph_param;
	return (aglyph->size);
 ...
 */
byte libaroma_font_glyph_draw(LIBAROMA_CANVASP dest,
		LIBAROMA_GLYPH aglyph_param, int x, int y, word color, byte flags,
		byte opacity) {
	_LIBAROMA_FONT_SLOT_CACHEP aglyph =
			(_LIBAROMA_FONT_SLOT_CACHEP) aglyph_param;
	if (!aglyph) {
	...
	if (flags & _LIBAROMA_TEXTCHUNK_BOLD) {
		FT_Outline_Embolden(&((FT_OutlineGlyph) fglyph)->outline,
				aglyph->size * 2);
	}
	...
	if (draw_w > 0) {
		...
		for (yy = 0; yy < bit->bitmap.rows; yy++) {
			...
			/* draw line */
			if (opacity == 0xff) {
				libaroma_alpha_multi_line(draw_w, dest_line, dest_line, color,
						src_line);
			} else {
				libaroma_alpha_multi_line(draw_w, tmp_dst, dest_line, color,
						src_line);
				libaroma_alpha_const(draw_w, dest_line, dest_line, tmp_dst,
						opacity);
			}
		}
		if (tmp_dst) {
...
byte libaroma_font_init() {
	...
	ALOGD("libaroma_font_init start .....");
	__libaroma_text_locker_init(0);
	ALOGD("__libaroma_text_locker_init end .....");
	_libaroma_font_lock(1);
	ALOGD("_libaroma_font_lock end .....");
	if (FT_Init_FreeType(&_libaroma_font_instance) == 0) {
#ifndef LIBAROMA_CONFIG_NOFONT_SUBPIXEL
		FT_Library_SetLcdFilter(_libaroma_font_instance, FT_LCD_FILTER_DEFAULT);
#endif
		/* cleanup font face */
		ALOGD("libaroma_font_init start cleanup font face ......");
