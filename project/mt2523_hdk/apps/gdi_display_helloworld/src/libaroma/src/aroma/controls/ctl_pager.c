...
#define _LIBAROMA_CTL_PAGER_TOUCH_CLIENT_WAIT 1500
...
static LIBAROMA_CONTROL_HANDLER _libaroma_ctl_pager_handler = { message
		:_libaroma_ctl_pager_msg, draw:_libaroma_ctl_pager_draw, focus:NULL,
...
byte _libaroma_ctl_pager_window_sync(LIBAROMA_WINDOWP win, int x, int y, int w,
		int h);
 ...
 */
byte _libaroma_ctl_pager_window_control_isvisible(LIBAROMA_WINDOWP win,
		LIBAROMA_CONTROLP cctl);
LIBAROMA_CANVASP _libaroma_ctl_pager_window_control_draw_begin(
		LIBAROMA_WINDOWP win, LIBAROMA_CONTROLP cctl);
static LIBAROMA_WINDOW_HANDLER _libaroma_ctl_pager_win_handler = { prefree:NULL,
postfree:NULL,
...
LIBAROMA_CANVASP _libaroma_ctl_pager_window_control_draw_begin(
	...
	if (!me->on_direct_canvas) {
		...
		c = libaroma_canvas_area(win->dc, cctl->x, cctl->y, cctl->w, cctl->h);
	} else {
		int xt = me->scroll_x;
		int x = cctl->x - xt;
		int y = cctl->y;
...
byte _libaroma_ctl_pager_window_updatebg(LIBAROMA_WINDOWP win) {
	...
	libaroma_canvas_setcolor(win->bg, libaroma_colorget(ctl, NULL)->window_bg,
			0xff);
 ...
 */
void _libaroma_ctl_pager_draw(LIBAROMA_CONTROLP ctl, LIBAROMA_CANVASP c) {
	/* internal check */_LIBAROMA_CTL_CHECK(
			_libaroma_ctl_pager_handler, _LIBAROMA_CTL_PAGERP,
	);
	ALOGI("==========>ctl pager draw<=========================")
	...
	/* draw window canvas */libaroma_mutex_lock(me->mutex);
	if (!me->on_direct_canvas) {
		if (me->win->dc) {
...
#pragma omp section
			{
#endif
		/* pretouched */libaroma_mutex_lock(me->mutex);
		if ((me->client_touch_start != 0) && (libaroma_tick()
				- me->client_touch_start
			...
			if (me->pretouched != NULL) {
				...
				if (me->win->touched->handler->message) {
					me->win->touched->handler->message(me->win->touched,
							&me->pretouched_msg);
				}
			}
		}
		...
		if (me->req_scroll_x != -1) {
			...
			if (me->req_scroll_x != me->scroll_x) {
				...
				int target_sz = me->scroll_x + move_sz+20;
				...
				ALOGD("=========onscroll x:%d  ========",me->scroll_x  ) ;
				if (me->controller) {
						...
						if (me->controller->handler) {
							if (me->controller->handler->onscroll) {
								...
								me->controller->handler->onscroll(
										me->controller->controller,
										me->controller->pager, scrollx,
										me->win->w, ctl->w, me->page_position);
								libaroma_mutex_lock(me->mutex);
							}
						}
					}
		...
		if (me->scroll_target_start > 0) {
				...
				if ((me->controller->pager == ctl)
					...
					if (me->controller->handler) {
						if (me->controller->handler->onscroll) {
							me->controller->handler->onscroll(
									me->controller->controller,
									me->controller->pager, me->scroll_x,
									me->win->w, ctl->w, me->page_position);
						}
					}
				}
			...
			if (state >= 1.0) {
				...
				ALOGD("=========onscroll_finish page_position x%d  ========", me->page_position ) ;
				if (me->controller) {
					if ((me->controller->pager == ctl)
							&& (me->controller->controller)) {
						if (me->controller->handler) {
 ...
 */
void _libaroma_ctl_pager_destroy(LIBAROMA_CONTROLP ctl) {
	/* internal check */_LIBAROMA_CTL_CHECK(
			_libaroma_ctl_pager_handler, _LIBAROMA_CTL_PAGERP,
	);
 ...
 */
dword _libaroma_ctl_pager_msg(LIBAROMA_CONTROLP ctl, LIBAROMA_MSGP msg) {
	/* internal check */_LIBAROMA_CTL_CHECK(
			_libaroma_ctl_pager_handler, _LIBAROMA_CTL_PAGERP, 0
	);
	dword ret = 0;
	...
	case LIBAROMA_MSG_TOUCH: {
		ALOGD("========= ctl pager touch x%d y%d========", msg->x,msg->y ) ;
		int x = msg->x;
		int y = msg->y;
		...
		if (msg->state == LIBAROMA_HID_EV_STATE_DOWN) {
			ALOGD("=========LIBAROMA_HID_EV_STATE_DOWN ctl pager touch x%d y%d ========", msg->x,msg->y ) ;
			libaroma_mutex_lock(me->mutex);
			...
			if (win->touched->handler->message) {
				...
				ALOGD("=========LIBAROMA_HID_EV_STATE_MOVE ctl pager touch x%d y%d  tell child========", msg->x,msg->y ) ;
			}
			if (msg->state == LIBAROMA_HID_EV_STATE_UP) {
				ALOGD("=========LIBAROMA_HID_EV_STATE_UP ctl pager touch x%d y%d  tell child========", msg->x,msg->y ) ;
				win->touched = NULL;
			}
		} else {
			if (msg->state == LIBAROMA_HID_EV_STATE_MOVE) {
			ALOGD("=========LIBAROMA_HID_EV_STATE_MOVE ctl pager touch x%d y%d ========", msg->x,msg->y ) ;
				libaroma_mutex_lock(me->mutex);
				if (me->max_scroll_x > 0) {
					...
					if (me->allow_scroll == 2) {
						...
						if ((abs(move_sz_y) >= scrdp) && (abs(move_sz_y)
								>= abs(move_sz))) {
				...
				ALOGD("=========LIBAROMA_HID_EV_STATE_UP ctl pager touch x%d y%d me->allow_scroll%d ========", msg->x,msg->y ,me->allow_scroll) ;
				libaroma_mutex_lock(me->mutex);
				if (me->allow_scroll) {
					...
					if (vel) {
						if (me->page_position * ctl->w < me->scroll_x) {
							if (vel > 0) {
								target_x++;
								velocity = ((float) vel) / 100.0;
								ALOGD("=========*****scroll to next  %d  velocity  %d *** ========", target_x ,vel) ;
							}
						} else if (me->page_position * ctl->w > me->scroll_x) {
							if (vel < 0) {
								target_x--;
								velocity = ((float) vel) / 100.0;
								ALOGD("=========*****scroll to pre %d  velocity  %d  *** ========", target_x ,vel) ;
							}

						}
					}
					libaroma_mutex_unlock(me->mutex);
					libaroma_ctl_pager_set_active_page(ctl, target_x, velocity);
					libaroma_mutex_lock(me->mutex);
				}
				if (me->client_touch_start || me->pretouched) {
					if (me->pretouched->handler->message) {
						...
						me->pretouched->handler->message(me->pretouched,
								&me->pretouched_msg);
						me->pretouched->handler->message(me->pretouched, msg);
					}
				}
				me->client_touch_start = 0;
				me->pretouched = NULL;
 ...
 */
byte libaroma_ctl_pager_set_active_page(LIBAROMA_CONTROLP ctl, int page_id,
		double velocity) {
	_LIBAROMA_CTL_CHECK(
			_libaroma_ctl_pager_handler, _LIBAROMA_CTL_PAGERP, 0
	);
 ...
 */
byte libaroma_ctl_pager_set_controller(LIBAROMA_CONTROLP ctl,
		LIBAROMA_CTL_PAGER_CONTROLLERP controller) {
	_LIBAROMA_CTL_CHECK(
			_libaroma_ctl_pager_handler, _LIBAROMA_CTL_PAGERP, 0
	);
 ...
 */
LIBAROMA_CONTROLP libaroma_ctl_pager(LIBAROMA_WINDOWP win, word id,
		int pager_number, int x, int y, int w, int h) {
	if (!win) {
	...
	/* init internal data */
	_LIBAROMA_CTL_PAGERP me = (_LIBAROMA_CTL_PAGERP) calloc(
			sizeof(_LIBAROMA_CTL_PAGER), 1);
	if (!me) {
	...
	LIBAROMA_CONTROLP ctl = libaroma_control_new(id, x, y, w, h,
			...
			(voidp) me, &_libaroma_ctl_pager_handler, NULL);
